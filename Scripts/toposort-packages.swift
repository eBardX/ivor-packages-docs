...
// discover inter-Ivor dependencies, then prints the packages in topological
// build order (each package after all Ivor packages it depends on).
//
...
func ivorPackages(in text: String) -> [String] {
    text.matches(of: #/github\.com/eBardX/(Ivor[^"]+)\.git/#)
        .map { String($0.output.1) }
}

let packages = ivorPackages(in: contents(of: umbrellaSwift))

// Collect inter-Ivor deps for each package from its checkout’s Package.swift.
var deps: [String: Set<String>] = .init(uniqueKeysWithValues: packages.map { ($0, []) })

for pkg in packages {
    let path = "\(checkoutsDir)/\(pkg)/Package.swift"

    for dep in ivorPackages(in: contents(of: path)) where dep != pkg && deps[dep] != nil {
        deps[pkg]!.insert(dep)
    }
}
